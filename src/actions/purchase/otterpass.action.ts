...
export async function buyOtterPass(data: { paymentId: string }) {
  try {
    ...
    if (payment.status !== PaymentStatus.VERIFIED) {
      console.warn("[OTTER PASS] Payment Failed");
    ...
    await saveOrderToFile("otter_pass", payload);
    return { success: true, message: "Otter Pass enabled successfully" };
  } catch (err) {
    console.log(err);
    return { success: false, message: "Failed to buy otter pass subscription" };
  }
}

export async function requestOtterPass(_amount: number) {
  ...
  const purchase = await db.purchase.create({
    data: {
      amount,
      purpose: PaymentPurpose.OTTER_PASS,
